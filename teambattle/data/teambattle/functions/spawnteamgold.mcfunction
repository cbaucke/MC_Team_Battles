...
execute unless entity @p[team=Gold, tag=Leader, distance=10..] run tellraw @p {"text":"Team leader dead or not found, try again later."}
execute unless entity @p[team=Gold, tag=Leader, distance=10..] run return 0
...
execute if score @p kitsAvailable >= constant one as @p run function teambattle:respawniron
...
tp @p @p[team=Gold, tag=Leader, distance=10..]